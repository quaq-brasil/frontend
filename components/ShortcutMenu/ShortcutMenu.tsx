...
export default function ShortcutMenu({
  ...
  function handleDeleteTemplate() {
    console.log("deleting: ", templateData?.id)
    setIsOpen(false)
    ...
    <div className="relative z-10 h-fit min-w-full w-fit pt-3 pl-3 overflow-scroll flex scrollbar-hide gap-3 items-center justify-start">
      {!openSizeOptions ? (
        ...
        <button
          ...
          className="min-w-[40px] h-10 rounded-full bg-slate-900 p-1 flex items-center justify-center"
        >
          <Crop className="w-5 h-5 text-white" weight="bold" />
        </button>
      )}
      <Popover
