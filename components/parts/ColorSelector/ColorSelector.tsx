...
export function ColorSelector(props: ColorSelectorProps) {
  ...
  return (
    <div
      className="pb-5 flex flex-col justify-center gap-[0.3125rem]
      min-w-[100%] bg-white 
    ...
    placeholder:text-slate-300 lg:text-[1.25rem] hover:outline-none focus:outline-none"
      ...
      <div className="pt-3">
        <div className="w-full flex flex-col justify-start px-3 lg:px-[1.125rem]">
          <span
            className={`w-[4.0625rem] h-[4.0625rem] rounded-full ${color}`}
          ></span>
        </div>
        <div className="w-full px-5 flex flex-row justify-between pt-5">
          {colors.map((color, index) => {
            return (
              <div
                className="w-[2.1875rem] h-[2.1875rem] lg:w-[2.8125rem] lg:h-[2.8125rem] rounded-full bg-slate-50"
                key={index}
              >
                <button
                  onClick={() => handleColorSelection(color)}
                  className={`w-[2.1875rem] h-[2.1875rem] lg:w-[2.8125rem] lg:h-[2.8125rem] rounded-full ${color}`}
                ></button>
              </div>
            );
          })}
